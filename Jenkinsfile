pipeline {
    ...
    environment {
    ACR_NAME = "cocoacr"
    registyUrl = "cocoacr.azurecr.io"
    IMAGE_NAME = "nodejswebbapp"
    IMAGE_TAG = "v1.0.0"
    registryCredential  = "claudia-acr"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/Reineclaudia/nodejs-webbapp.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
